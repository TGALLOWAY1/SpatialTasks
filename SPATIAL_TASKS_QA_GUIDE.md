...
2. Clicks the ✨ wand button in the top bar → popover lists two strategies (**Tidy** / **Grid**) and a Tidy-direction toggle (Top-down / Left-right).
3. Picks **Tidy** (default). Nodes animate (~450 ms ease-out) into a dagre-computed Sugiyama layout: roots at the top (or left in LR), dependencies flow toward leaves, no overlapping rectangles. When the subset has no internal edges, Tidy falls back to packing each weakly-connected component into its own grid block side-by-side.
4. Picks **Grid**: nodes animate into clean rows/columns sorted by their current (y, x) position so a node at the top-left ends up in row 0 / col 0 — mental map preserved.
5. Presses **Ctrl+Z** once → exact prior positions restored (single undo step).
6. Right-clicks the canvas pane → "Auto-Organize ▸" submenu offers Tidy (top-down), Tidy (left-right), Grid.
7. Selects a subset of nodes, opens the wand popover, ticks "Apply to selection only" → only the selected nodes move; unselected nodes are byte-identical and the camera does not jump (the result is anchored inside the selection's original bounding box).
8. On touch: opens the TopBar overflow menu → "Auto-Organize" section lists Tidy + Grid plus a TB/LR toggle row.
9. Re-opening the popover highlights the last-used strategy as "Last used" (persisted in `settings.preferredLayoutStrategy`); orientation persists in `settings.preferredLayoutOrientation`.
10. A user upgrading from a previous build (where they last picked Cluster, Hierarchy, or Flow) sees Tidy as the active strategy after reload (Cluster/Hierarchy → Tidy/TB, Flow → Tidy/LR).

**What could go wrong**:
- Animation janks on graphs with many nodes (>200) or during concurrent drag.
- Selection-only run moves unselected nodes (sentinel handling bug) or the camera jumps even though the result fits in view.
- Resized container nodes overlap their neighbours after a layout (live `width`/`height` from `reactFlowInstance.getNodes()` not consulted).
- Undo fails to restore exact pre-organize positions (batchUpdatePositions commit order issue).
- Persisted legacy strategies (`'cluster'`/`'hierarchy'`/`'flow'`) bleed through and crash typing — must be migrated on rehydrate and on Supabase hydration.
- Popover stays open after applying a layout, or doesn't close on outside click.
- Preferred strategy or orientation not persisted after reload.
...
| QA-A018b | Task Editing | Expanded notes editor uses larger desktop and mobile viewport | A node has notes open in NotesEditor | 1. Open notes on desktop and click expand. 2. Verify the expanded editor fills most of the viewport width/height. 3. Repeat on a phone-sized viewport. | Expanded mode opens as a large modal with a tall textarea and enough space for multi-paragraph editing on both desktop and mobile. Save/close controls remain visible. | Regression check for overly small expanded notes modal; verify keyboard does not hide the footer on touch devices | P1 |
...
| QA-A040 | Auto-Organize | Tidy / top-down chain | A graph with chain A→B→C→D | 1. Drag nodes to overlap / scatter 2. Open ✨ Auto-Organize, ensure direction is Top-down 3. Pick "Tidy" | Nodes animate (~450 ms) into a top-down dagre layout: y strictly increases from A to D; same-rank siblings share y; no overlapping rectangles | Final AABBs must not overlap (dagre guarantee); popover closes after selection | P0 |
| QA-A041 | Auto-Organize | Tidy / left-right chain | Same chain | 1. Open ✨ Auto-Organize, switch direction to Left-right 2. Pick "Tidy" | x strictly increases from A to D; siblings share x; same animation feel | Orientation toggle persists in `settings.preferredLayoutOrientation` | P1 |
| QA-A042 | Auto-Organize | Tidy preserves disconnected subgraphs | Two unconnected pairs (A→B), (C→D) | 1. Open ✨ Auto-Organize 2. Pick "Tidy" | Each pair lays out as its own mini-tree; the two pairs are placed side-by-side and never interleave | Disconnected components must not merge into a single rank-0 row | P1 |
| QA-A043 | Auto-Organize | Grid preserves mental map | A graph with 9 unconnected nodes | 1. Drag one node to clearly the top-left 2. Open ✨ Auto-Organize 3. Pick "Grid" | 3×3 packing; the dragged node ends up in row 0, col 0; ordering reflects current (y, x), NOT random by UUID | No overlaps; grid cells use measured node sizes | P1 |
| QA-A044 | Auto-Organize | Undo restores exact prior positions | Any graph after running Auto-Organize | 1. Note positions of 2-3 nodes 2. Run Tidy or Grid 3. Press Ctrl+Z | All nodes return to their exact pre-organize positions | Single undo step via `batchUpdatePositions` commit | P0 |
| QA-A045 | Auto-Organize | Cyclic edges still lay out cleanly | A graph with a cycle (A→B→C→A) | 1. Pick Tidy | dagre handles the cycle via greedy feedback-arc-set; all nodes laid out without throw or infinite recursion | No special-case fallback; output is overlap-free | P1 |
| QA-A046 | Auto-Organize | Selection-only scope respects anchor | A graph with 10 nodes | 1. Select 3 nodes 2. Open ✨ Auto-Organize 3. Tick "Apply to selection only" 4. Pick Tidy or Grid | Only the 3 selected nodes are repositioned; the result is anchored inside the selection's original bounding box; the camera does not jump (fitView only fires when the result spans outside the visible flow rect) | Other 7 node positions byte-identical post-run | P1 |
| QA-A047 | Auto-Organize | Resized container does not overlap neighbors | Place a container resized to ~400×250 next to two action nodes with edges between them | 1. Pick Tidy | The container's measured size is honored; nothing overlaps it | Live `width`/`height` from `reactFlowInstance.getNodes()` must be passed via `sizeOverrides` | P1 |
| QA-A048 | Auto-Organize | Context menu submenu | A graph with 2+ nodes | 1. Right-click an empty pane area 2. Hover "Auto-Organize ▸" | Submenu shows 3 entries: Tidy (top-down), Tidy (left-right), Grid | Submenu stays within viewport | P1 |
| QA-A049 | Auto-Organize | Idempotent re-run | A graph with 10+ nodes | 1. Run Tidy 2. Wait for animation to settle 3. Run Tidy again | Second run produces zero net movement (positions stable) | `computeLayout` is pure and dagre is deterministic | P2 |
| QA-A050 | Auto-Organize | Preferred strategy + orientation persist | Last used Tidy / left-right | 1. Run Tidy/LR 2. Reload page 3. Open ✨ Auto-Organize | "Tidy" shows the "Last used" pill; orientation toggle is set to Left-right | Persisted via `settings.preferredLayoutStrategy` and `settings.preferredLayoutOrientation` (localStorage + Supabase sync) | P2 |
| QA-A051 | Auto-Organize | Legacy strategy migration | localStorage holds `preferredLayoutStrategy: 'flow'` from a previous build | 1. Edit `localStorage` directly to set the legacy value 2. Reload page 3. Open ✨ Auto-Organize | Strategy reads as "Tidy" with orientation Left-right (Cluster/Hierarchy/legacy values map to Tidy/Top-down) | Migration runs in `onRehydrateStorage` and `hydrateFromSupabase` | P2 |
| QA-A052 | Auto-Organize | Mobile overflow menu entries | Touch device | 1. Open the top-bar overflow menu (⋮) 2. Scroll to "Auto-Organize" section | Two strategy buttons (Tidy / Grid) plus a TB/LR toggle row; tapping a strategy runs the layout and closes the menu | Buttons meet 44 px min touch target | P2 |
| QA-A053 | Auto-Organize | No-op on empty or single-node graph | 0 or 1 nodes on the canvas | 1. Run any Auto-Organize strategy | No animation, no error, no state change | Must not throw or create a stray undo entry | P2 |
...
| QA-B032f | Execution mode | Collapse and expand the step-detail panel | Execution mode active; inside a container with substeps; panel is visible | 1. Tap the `PanelRightClose` icon in the panel header to collapse it. 2. Tap the "Step Details" tab that appears on the right edge to expand it again. 3. Toggle once more in either direction. | Panel collapses to a vertical tab on the right and expands back to the full panel. No crash, no "Something went wrong" error screen, no console hook-order warnings. Tasks/Notes view, scroll position, and any in-progress notes edit are preserved across the toggle. | Regression test for React error #300 caused by hooks being called after conditional returns in `StepDetailPanel.tsx` | P0 |
...
| QA-B035 | Focus view | Enter Focus View shows next actionable task | Project with several action nodes; first one has both notes and images | 1. Click the Crosshair icon in TopBar to switch to Focus View. | View renders the first actionable leaf task from the currently active project. If it has images, hero image fills top half (~40-45vh); otherwise notes fill the screen. Title and notes are shown below. Status pill is visible at the bottom. Header shows "Task 1 of N". | Entering Focus View should reload the active project's root graph before picking the next actionable task; "No image for this task" pill shown when images array empty | P0 |
...
### Group C: Projects, Persistence & AI (41 cases)
...
| QA-C018 | Markdown Import | Import a well-formed markdown plan | App loaded | 1. Open MarkdownImporter 2. Paste a markdown plan with `#`, `##`, and `- Task:` bullets (optionally with `(parallel)` and `depends_on:`) 3. Confirm import | Headings become container nodes; `- Task:` bullets become task nodes nested appropriately; `Task:` / `Subtask:` prefixes are stripped from labels; a new project is created | Nested lists should map to hierarchy; formatting artifacts (**, ##) should be stripped from node labels | P1 |
...
| QA-C034 | Structured Plan Import | Parallel marker skips sequential edges | App loaded | 1. Open "Import Implementation Plan" 2. Paste `## Build (parallel)\n- Task: A\n- Task: B` under a `#` title 3. Parse & Review → Create on Canvas 4. Drill into the "Build" container | Container opens to a child graph with two action nodes ("A", "B") and **no edge** between them; nodes are stacked vertically | Non-parallel siblings must still get the sequential chain; verify by comparing with a non-parallel phase | P1 |
| QA-C035 | Structured Plan Import | depends_on creates cross-edges | App loaded | 1. Paste `## Launch\n- Task: Build\n- Task: Ship\n  depends_on: Build` 2. Create on Canvas 3. Drill into "Launch" | Edge exists from "Build" → "Ship". No duplicate sequential edge is created when depends_on is declared | Label matching is case-insensitive; comma-separated list (`depends_on: A, B`) produces multiple edges | P1 |
| QA-C036 | Structured Plan Import | Unresolved depends_on shows warning | App loaded | 1. Paste `## Step\n- Task: Real\n- Task: Other\n  depends_on: NotARealLabel` 2. Create on Canvas | Success toast appears for creation AND an info toast per unresolved reference; the rest of the structure still imports correctly (no crash) | Warning text should name the offending task and the missing label | P1 |
| QA-C037 | Structured Plan Import | Templates tab loads starter markdown | App loaded | 1. Open importer → Templates tab 2. Click "Project Plan" card body (not the copy icon) | Template markdown appears pre-filled in the Paste tab; active tab switches to Paste; success toast confirms which template loaded | User can edit the markdown before clicking Parse & Review | P2 |
| QA-C038 | Structured Plan Import | All 6 templates parse end-to-end with no warnings | App loaded | 1. For each of Coding Agent Process / Project Plan / Learning Path / General Workflow / Bug Fix / Research Spike: load template → Parse & Review → Create on Canvas | Each template commits a new project with the expected container/action structure (4-5 phases, 12-19 child tasks). At least one phase per template (except Bug Fix) is marked parallel and produces no sequential edges between siblings; depends_on annotations produce correct cross-edges | No parse warnings for the shipped templates. The HTML `<!-- FORMAT LEGEND -->` block at the top of each template must be silently ignored by the parser (no stray nodes, no warnings) | P2 |
| QA-C038a | Structured Plan Import | Template quick-copy icon copies markdown | App loaded, clipboard permission granted | 1. Open importer → Templates tab 2. Click the copy icon (not the card) on any template 3. Paste into any text editor | Clipboard contains the full template markdown including the FORMAT LEGEND comment; success toast names the template; copy icon swaps to a green check mark for ~2s; the template does NOT load into the Paste tab and the active tab does not change | Click event must not propagate to the parent card. Markdown should be identical to what the card's click would load. Useful for handing the template to a coding agent (Claude Code / Cursor / ChatGPT) | P2 |
| QA-C039 | Structured Plan Import | AI Prompt tab copies substituted prompt | App loaded, clipboard permission granted | 1. Open importer → AI Prompt tab 2. Type "Launch a podcast" in the goal field 3. Click Copy Prompt 4. Paste into any text field | Clipboard contains the prompt template with `[USER GOAL]` replaced by "Launch a podcast"; button briefly shows "Copied!" | Empty goal still copies the prompt with the literal `[USER GOAL]` placeholder | P2 |
| QA-C040 | Structured Plan Import | AI Prompt → Open Generate Flow bridges to FlowGenerator | geminiApiKey set or unset (test both) | 1. Open importer → AI Prompt tab 2. Click "Prefer in-app generation? Open Generate Flow with AI" | MarkdownImporter closes; FlowGenerator (Gemini-powered) opens in its current state — no data leaked between modals | Confirm existing FlowGenerator behavior is unchanged (no regressions) | P2 |
| QA-C041 | Structured Plan Import | Import is a single undo step | App loaded with no active project change in progress | 1. Use the importer to create a project (Templates → Project Plan → Create) 2. Press Ctrl/Cmd+Z once | Project, root graph, and all child graphs disappear in a single undo; canvas returns to the previous active project | Re-do (Ctrl+Shift+Z) restores the full project identically | P1 |
