...
| QA-C037 | Structured Plan Import | Templates tab loads starter markdown | App loaded | 1. Open importer → Templates tab 2. Click "Project Plan" | Template markdown appears pre-filled in the Paste tab; active tab switches to Paste; success toast confirms which template loaded | User can edit the markdown before clicking Parse & Review | P2 |
| QA-C038 | Structured Plan Import | All 3 templates parse end-to-end | App loaded | 1. For each of Project Plan / Learning Path / General Workflow: load template → Parse & Review → Create on Canvas | Each template commits a new project with the expected container/action structure, including at least one parallel group and one depends_on edge | No parse warnings for the shipped templates (they should be well-formed) | P2 |
